//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 02_LoadCell_Sensor_5kg_Calibration
//----------------------------------------Including the libraries.
#include "HX711.h"
#include <EEPROM.h>
//----------------------------------------

//----------------------------------------Defines the connected PIN between HX711 and Arduino UNO.
  ...
  if(Serial.available()) {
    ...
    if (inChar == 'C') {
      if (LOADCELL_HX711.is_ready()) {
        ...
        Serial.println(); 
        Serial.println("Save calibration values to EEPROM.");
        EEPROM.put(0, CALIBRATION_FACTOR); // Saves calibration values to EEPROM. Format : EEPROM.put(address, data).
        delay(500);

        Serial.println(); 
        Serial.println("Load calibration values from EEPROM.");
        float EEPROM_CALIBRATION_FACTOR; 
        EEPROM.get(0, EEPROM_CALIBRATION_FACTOR); // Load calibration values stored in EEPROM. Format : EEPROM.get(address, data).
        delay(500);

        Serial.println(); 
        Serial.println("Set the scale with the CALIBRATION_FACTOR value.");
        LOADCELL_HX711.set_scale(EEPROM_CALIBRATION_FACTOR); // Set the scale with the CALIBRATION_FACTOR value.
        delay(100);
        
        Serial.println(); 
        Serial.print("CALIBRATION_FACTOR : ");
        Serial.println(EEPROM_CALIBRATION_FACTOR);
        delay(2000);
